...
class Recommender:
    _model_file = open(os.path.dirname(os.path.realpath(__file__)) +"/final", "rb")
    _users_file = open(os.path.dirname(os.path.realpath(__file__)) +"/known_users", "rb")
    _user_converter = {
        'User-ID': int,
        'Age': int,
        'City': str,
    ...
    def cold_start_recommend(self, age: int, country: str, has_rated_items: List[str] = [], n_items: int = 10):
        similar_users = self.cold_start_similar_users(age, country)["User-ID"].values[:300]
